...
/*bool ModelState::setState(double *qj_t, double *qj_dot_t, double *vb_t)
{
  ...
  std::string sqj_string     = int_array_to_string(qj_t, nDof);
  std::string sqj_dot_string = int_array_to_string(sqj_dot, nDof);
  std::string svb_string     = int_array_to_string(svb, 6);

  mexPrintf("check state value after update.\n");
  mexPrintf(sqj_string.c_str());
  mexPrintf("\n");
  mexPrintf(sqj_dot_string.c_str());
  mexPrintf("\n");
  mexPrintf(svb_string.c_str());
  mexPrintf("\n");
#endif
